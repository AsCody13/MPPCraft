...
function Client(uri, proxy, token) {
...
Client.prototype.connect = function () {
	...
	this.ws.addEventListener("open", function (evt) {
		...
		self.sendArray([{ m: "hi", token: self.token }]); // Changes for token system
...
Client.prototype.bindEventListeners = function () {
	...
	// this.on("b", function(msg) { // broken, so cancel changes
	// 	self.sendArray([{m:'hi',token:self.token,code:'broken','🐈':1}])
	// });
};
